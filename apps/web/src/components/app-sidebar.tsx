...
export function AppSidebar({ collapsed, onToggle, onNavigate, embedded }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "flex h-full flex-col bg-sidebar text-sidebar-foreground shadow-sm transition-[width] duration-300 ease-in-out",
        collapsed ? "w-[72px]" : "w-[260px]",
      )}
    >
      {/* Brand */}
      <div className={cn("flex h-[60px] shrink-0 items-center border-b border-sidebar-border", collapsed ? "justify-center px-3" : "gap-3 px-5")}>
        <Link href="/dashboard" className="flex items-center gap-3 rounded-lg" onClick={onNavigate}>
          {schoolProfile?.logo_url ? (
            <img src={schoolProfile.logo_url} alt={activeSchool?.school_name ?? "School"} className="h-10 w-10 shrink-0 rounded-lg bg-white object-contain shadow-sm" />
          ) : (
            <Image src="/clearisbg.png" alt="Clearis" width={1536} height={1024} priority className="h-10 w-auto shrink-0 rounded-lg object-contain" />
          )}
          {!collapsed && (
      ...
      {!collapsed && activeSchool && (
        <div className="shrink-0 px-3 pt-3">
          <div className="rounded-xl border border-sidebar-border bg-sidebar-accent px-3.5 py-2.5">
            <p className="truncate text-[12px] font-semibold text-sidebar-foreground">{activeSchool.school_name}</p>
      ...
      <nav className="scrollbar-thin flex-1 space-y-6 overflow-y-auto px-3 pt-4 pb-2">
        {sections.map((section) => (
          <div key={section.label}>
            {!collapsed && (
              <p className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-sidebar-muted">
                {section.label}
              </p>
            )}
      ...
      <button
        ...
        className={cn(
          "mx-3 mb-2 flex items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-sidebar-muted transition-colors hover:bg-sidebar-accent hover:text-sidebar-foreground",
      ...
      <div className="shrink-0 border-t border-sidebar-border p-3">
        <div className={cn("rounded-xl", collapsed ? "flex justify-center" : "bg-sidebar-accent p-2.5")}>
          <div className={cn("flex items-center", collapsed ? "flex-col" : "gap-2.5")}>
            <Link href="/settings" className="rounded-full" onClick={onNavigate} title={collapsed ? user?.full_name ?? "Profile" : undefined}>
              <Avatar name={user?.full_name} initials={initials} className="h-8 w-8 bg-gradient-to-br from-indigo-500 to-indigo-700 text-[11px] text-white ring-2 ring-white shadow-sm" />
            </Link>
            {!collapsed && (
              <div className="min-w-0 flex-1">
                <p className="truncate text-[12px] font-semibold text-sidebar-foreground">{user?.full_name}</p>
          ...
          {!collapsed && memberships.length > 1 && (
            <div className="mt-2">
              ...
              <select
                ...
                className="w-full rounded-lg border border-sidebar-border bg-white px-2.5 py-1.5 text-[11px] font-medium text-sidebar-foreground outline-none focus-visible:ring-2 focus-visible:ring-sidebar-primary"
              >
                {memberships.map((m) => (
                  <option key={m.school_id} value={m.school_id} className="bg-white text-sidebar-foreground">
                    {m.school_name}
                  </option>
                ))}
              </select>
          ...
          {!collapsed && (
            <button
              ...
              className="mt-2 flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-[11px] font-medium text-sidebar-muted transition-colors hover:bg-sidebar-accent hover:text-sidebar-foreground"
            >
          ...
          {collapsed && (
            <button
              onClick={handleLogout}
              title="Sign out"
              className="mt-2 flex w-full items-center justify-center rounded-lg px-2 py-1.5 text-sidebar-muted transition-colors hover:bg-sidebar-accent hover:text-sidebar-foreground"
            >
