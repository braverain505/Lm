...
export function SearchPalette() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex h-8 items-center gap-2 rounded-lg border border-border/40 bg-transparent px-3 text-[12px] text-muted-foreground/50 transition-all hover:border-border/60 hover:text-foreground sm:w-48"
      >
              ...
              {results.length === 0 ? (
                ...
                results.map((item) => (
                  <Link
                    ...
                    className={cn(
                      "flex items-center gap-3 rounded-lg px-3 py-2.5 text-[12px] transition-colors hover:bg-muted/30",
